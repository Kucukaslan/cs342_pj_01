...
int main(int argc, char **argv) {
  ...
  int intervalwidth =
      10; // • Max value for <intervalwidth> can be 1000000. Min value is 1.
  int intervalstart = 0;
  ...
  for (i = 0; i < N; ++i) {
    ...
    if (child_pids[i] < 0) {
      ...
      // struct ChildParentItem child(char* filename, int intervalcount, int
      // intervalwidth, int intervalstart);
  ...
  struct ServerClientItem serverClientItem = {
      .interval = -1, .interval_frequency = -1, .status = CLIENT_CONTINUE};
  ...
  serverClientItem.interval_frequency = -1;
...
void child(char *filename, int intervalcount, int intervalwidth,
  ...
  printf("Parameters are %s, %d, %d, %d\n", filename, intervalcount,
         intervalwidth, intervalstart);
  int i = 0;
  ...
  i = 0;
  struct ChildParentItem childParentItemPtr = {.status = CHILD_CONTINUE,
                                               .pid = -1,
                                               .interval = -1,
  ...
  if (n == -1) {
    ...
    */
    ...
    if (itemptr->interval >= 0 && itemptr->interval_frequency >= 0)
      interval_freq[itemptr->interval] =
          interval_freq[itemptr->interval] + itemptr->interval_frequency;
  }
  free(bufptr);
  return status;
}
